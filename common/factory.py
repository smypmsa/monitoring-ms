...
class MetricFactory:
    ...
    @classmethod
    def create_metrics(
        cls, blockchain_name: str, config: MetricConfig, **kwargs
    ) -> List[BaseMetric]:
        if blockchain_name not in cls._registry:
            raise ValueError(
                f"No metric classes registered for blockchain '{blockchain_name}'. Available blockchains: {list(cls._registry.keys())}"
            )
